...
const BUILD_ID = '2026-04-23-codex-7';
document.title = `Wezone (${BUILD_ID})`;
...
renderer.setClearColor(0x8ecdf2);
...
if (dom.menuScreen) dom.menuScreen.style.display = 'grid';
...
function makeWoodTexture() {
  ...
  grad.addColorStop(1, '#6a3f1b');
  ctx.fillStyle = grad;
  ctx.fillRect(0, 0, 256, 256);
  for (let i = 0; i < 120; i += 1) {
    ctx.strokeStyle = `rgba(60,30,10,${0.05 + Math.random() * 0.1})`;
    ctx.beginPath();
    ctx.moveTo(0, i * 2 + Math.random() * 10);
    ctx.bezierCurveTo(80, i * 2, 180, i * 2 + 16, 256, i * 2 + Math.random() * 12);
    ctx.stroke();
  }
...
function makeTextSprite(text, y, style = { bg: 'rgba(10,26,40,0.85)', fg: '#effbff' }) {
  ...
  ctx.fillRect(8, 18, 496, 90);
  ctx.strokeStyle = 'rgba(172,232,255,0.9)';
  ctx.lineWidth = 3;
  ctx.strokeRect(8, 18, 496, 90);
  ctx.fillStyle = style.fg;
  ctx.textAlign = 'center';
  ctx.font = '44px Inter';
  ctx.fillText(text.slice(0, 24), 256, 76);
  const tex = new THREE.CanvasTexture(canvas);
  tex.colorSpace = THREE.SRGBColorSpace;
...
function buildAvatar(data) {
  ...
  const gun = new THREE.Mesh(
    new THREE.BoxGeometry(0.15, 0.12, 0.7),
    new THREE.MeshStandardMaterial({ color: 0xb1d8ff, metalness: 0.95, roughness: 0.2 })
  );
  gun.position.set(0.48, 2.15, 0.45);
  gun.rotation.y = Math.PI * 0.08;
  group.add(gun);
...
function setChatBubble(player, text) {
  if (player.userData.chatText === text && player.userData.chat) return;
  player.userData.chatText = text;
...
function upsertPlayer(data) {
  ...
  p.userData.deadUntil = data.deadUntil || 0;
  const alive = p.userData.deadUntil <= Date.now();
  p.visible = alive;
...
function updatePhysics(dt) {
  ...
  if (!me) return;
  if ((me.userData.deadUntil || 0) > Date.now()) return;

  for (const [id, other] of players) {
    if (id === selfId || !other.visible) continue;
    other.position.lerp(other.userData.targetPos, 0.25);
    const deltaYaw = other.userData.targetRotY - other.rotation.y;
    other.rotation.y += deltaYaw * 0.25;
  }
  ...
  for (const [id, other] of players) {
    if (id === selfId || !other.visible) continue;
    const delta = me.position.clone().sub(other.position);
    delta.y = 0;
    const d = delta.length();
    if (d > 0 && d < 1.3) {
      delta.normalize();
      const push = (1.3 - d) * 0.5;
      me.position.addScaledVector(delta, push);
    }
  }
  ...
  for (const prop of props.values()) {
    ...
    if (prop.userData.box.intersectsSphere(meBall)) {
      const away = prop.position.clone().sub(me.position).setY(0);
...
async function joinGame() {
  ...
  dom.playBtn.textContent = 'Joining...';
  ...
  try {
    ...
    dom.playBtn.textContent = 'Failed to connect. Retry';
...
window.addEventListener('mousedown', () => {
  if (!playing || !selfId) return;
  const me = players.get(selfId);
  if (!me || (me.userData.deadUntil || 0) > Date.now()) return;
  const now = performance.now();
  if (now - lastShotAt < 160) return;
  lastShotAt = now;

  let victimId = null;
  let best = Infinity;
  const origin = me.position.clone().add(new THREE.Vector3(0, 2.1, 0));
  const forward = new THREE.Vector3(Math.sin(me.rotation.y), 0, Math.cos(me.rotation.y)).normalize();

  for (const [id, other] of players) {
    if (id === selfId || !other.visible) continue;
    const to = other.position.clone().add(new THREE.Vector3(0, 2, 0)).sub(origin);
    const proj = to.dot(forward);
    if (proj < 0 || proj > 45) continue;
    const nearest = origin.clone().addScaledVector(forward, proj);
    const miss = nearest.distanceTo(other.position.clone().add(new THREE.Vector3(0, 2, 0)));
    if (miss < 1.0 && proj < best) {
      best = proj;
      victimId = id;
    }
  }

  if (victimId) postJSON('/api/hit', { id: selfId, victimId }).catch(() => {});
});
